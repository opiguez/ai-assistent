  ...
  server.registerPrompt(
    ...
    async ({
      ...
      if (executeStep && session.mode === 'CHUNK_PROCESSING') {
        ...
        currentTaskText =
          'Очередь задач пуста или находится в обычном режиме чата. Пожалуйста, сначала отправьте ТЗ архитектору (@plan), чтобы он сформировал задачи с помощью инструмента save_tasks_queue.';
      }
