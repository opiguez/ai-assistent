...
4. БИНАРНЫЕ СОСТОЯНИЯ — BOOLEAN:
   ...
   Анализируй последовательность статусов: если среди N значений есть пара "X" и "не X" (например, "Ожидает оплаты" / "Оплачен") — это одно бинарное состояние, выноси его в BOOLEAN (isPaid). Остальные статусы, не образующие бинарную пару (например, "В работе", "На упаковке", "Отгружен"), остаются в SELECTION-поле.
...
5а. Группы и именование справочников:
    Группу справочников создавай с именем модуля (напр. `spaOrderAccountingData`). parentGroupId по умолчанию — корневой workspace `/modules/_rdm/workspaces/_rdm_workspace`.
