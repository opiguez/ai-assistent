...
4в. Lifecycle:
    ...
    `data_create_data_type` (обычный тип) автоматически резолвит lifecycle
    из родительского модуля. Хендлер запрашивает `module.lifecycles[0].name`
    и строит путь `/modules/{moduleName}/lifecycles/{lifecycleName}`.
    Поддерживает как реальный ID модуля, так и `PENDING_MODULE_ID`.
...
4г. childrenTypes (ограничение дочерних типов):
    ...
    **Когда использовать:**
    - Тип-контейнер (orderBatch) → childrenTypes = [ID дочернего типа (spaOrder)] - бери из контекста или получи
...
4д. СВЯЗАННЫЕ СУЩНОСТИ С АТРИБУТАМИ (Data Type):
     Если ТЗ описывает сущность, которая имеет собственные поля и входит
     в состав другой — это Data Type (не справочник). Конкретный тип
     (обычный data_create_data_type или BPMN data_create_bpmn_data_type)
     определяй по контексту ТЗ:
     - Если есть workflow/этапы/процесс — BPMN
     - Если просто набор полей — обычный
     - Если ТЗ говорит о родительских/дочерних отношениях — canHaveChildren
     Размещай в том же модуле.
     **При использовании `data_create_data_type` — lifecycle резолвится
     автоматически (см. п.4в).**
     НЕ добавляй поле-ссылку (DataObjects) на родительский тип, если
     задача этого явно не требует — связь может быть реализована позже.
