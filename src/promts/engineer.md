...
4б. BOOLEAN ДЛЯ БИНАРНЫХ СОСТОЯНИЙ:
    ...
    Анализируй последовательность статусов: если среди N значений есть пара "X" и "не X" (например, "Ожидает оплаты" / "Оплачен") — это одно бинарное состояние, выноси его в BOOLEAN-поле (isPaid). Остальные статусы, не образующие бинарную пару (например, "В работе", "На упаковке", "Отгружен"), остаются в SELECTION-поле.
